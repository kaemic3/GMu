...
public:
    ...
    uint8_t nop(); uint8_t add_a_a(); uint8_t add_a_abs_hl(); uint8_t add_a_b();  uint8_t add_a_c();  uint8_t add_a_d(); uint8_t add_a_e(); uint8_t add_a_h(); uint8_t add_a_l(); uint8_t add_hl_bc();     uint8_t add_hl_de();  uint8_t add_hl_hl(); uint8_t add_hl_sp(); uint8_t adc_a_abs_hl(); uint8_t adc_a_a(); uint8_t adc_a_b(); uint8_t adc_a_c();  uint8_t adc_a_d(); uint8_t adc_a_e(); uint8_t adc_a_h(); uint8_t adc_a_l(); uint8_t ccf(); uint8_t cpl(); uint8_t daa(); uint8_t dec_a(); uint8_t dec_abs_hl();  uint8_t dec_b();              uint8_t dec_bc();           uint8_t dec_c();           uint8_t dec_d();           uint8_t dec_de();  uint8_t dec_e(); uint8_t dec_h(); uint8_t dec_hl(); uint8_t dec_l(); uint8_t dec_sp(); uint8_t halt();
