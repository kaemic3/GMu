...
public:
    ...
    uint8_t nop();           uint8_t add_hl_bc();     uint8_t add_hl_de();  uint8_t add_hl_hl(); uint8_t add_hl_sp(); uint8_t cpl(); uint8_t daa(); uint8_t dec_abs_hl();  uint8_t dec_b();              uint8_t dec_bc();           uint8_t dec_c();           uint8_t dec_d();           uint8_t dec_de();  uint8_t dec_e(); uint8_t dec_h(); uint8_t dec_hl(); uint8_t dec_l();
     uint8_t inc_abs_hl(); uint8_t inc_b();         uint8_t inc_bc();        uint8_t inc_c();           uint8_t inc_d();              uint8_t inc_de();           uint8_t inc_e();   uint8_t inc_h(); uint8_t inc_hl();  uint8_t inc_l();  uint8_t inc_sp();   uint8_t jr_r8();   uint8_t jr_c_r8();     uint8_t jr_nc_r8();      uint8_t jr_nz_r8();    uint8_t jr_z_r8();   uint8_t ld_a_abs_hli();     uint8_t ld_a_abs_bc();
