...
public:
    ...
    uint8_t nop(); uint8_t add_a_a(); uint8_t add_a_abs_hl(); uint8_t add_a_b();  uint8_t add_a_c();  uint8_t add_a_d(); uint8_t add_a_d8(); uint8_t add_a_e(); uint8_t add_a_h(); uint8_t add_a_l(); uint8_t add_hl_bc();     uint8_t add_hl_de();  uint8_t add_hl_hl(); uint8_t add_hl_sp(); uint8_t adc_a_abs_hl(); uint8_t adc_a_a(); uint8_t adc_a_b(); uint8_t adc_a_c();  uint8_t adc_a_d(); uint8_t adc_a_d8(); uint8_t adc_a_e(); uint8_t adc_a_h(); uint8_t adc_a_l(); uint8_t and_a(); uint8_t and_abs_hl(); uint8_t and_b(); uint8_t and_c(); uint8_t and_d(); uint8_t and_e(); uint8_t and_h(); uint8_t and_l(); uint8_t call_a16(); uint8_t call_c_a16(); uint8_t call_nc_a16(); uint8_t call_nz_a16(); uint8_t call_z_16(); uint8_t ccf(); uint8_t cp_a(); uint8_t cp_abs_hl(); uint8_t cp_b(); uint8_t cp_c(); uint8_t cp_d(); uint8_t cp_e(); uint8_t cp_h(); uint8_t cp_l(); uint8_t cpl(); uint8_t daa(); uint8_t dec_a(); uint8_t dec_abs_hl();  uint8_t dec_b();              uint8_t dec_bc();           uint8_t dec_c();           uint8_t dec_d();           uint8_t dec_de();  uint8_t dec_e(); uint8_t dec_h(); uint8_t dec_hl(); uint8_t dec_l(); uint8_t dec_sp(); uint8_t halt();
    uint8_t inc_a(); uint8_t inc_abs_hl(); uint8_t inc_b();         uint8_t inc_bc();        uint8_t inc_c();           uint8_t inc_d();              uint8_t inc_de();           uint8_t inc_e();   uint8_t inc_h(); uint8_t inc_hl();  uint8_t inc_l();  uint8_t inc_sp();uint8_t jp_a16(); uint8_t jp_c_a16(); uint8_t jp_nc_a16(); uint8_t jp_nz_a16(); uint8_t jp_z_a16(); uint8_t jr_r8();   uint8_t jr_c_r8();     uint8_t jr_nc_r8();      uint8_t jr_nz_r8();    uint8_t jr_z_r8(); uint8_t ld_a_a(); uint8_t ld_a_abs_hl(); uint8_t ld_a_b(); uint8_t ld_a_c(); uint8_t ld_a_d(); uint8_t ld_a_d8(); uint8_t ld_a_e(); uint8_t ld_a_h(); uint8_t ld_a_l(); uint8_t ld_a_abs_bc();
    ...
    uint8_t ld_de_d16(); uint8_t ld_e_a();  uint8_t ld_e_abs_hl();  uint8_t ld_e_b(); uint8_t ld_e_c(); uint8_t ld_e_d(); uint8_t ld_e_d8(); uint8_t ld_e_e(); uint8_t ld_e_h(); uint8_t ld_e_l(); uint8_t ld_h_a(); uint8_t ld_h_abs_hl(); uint8_t ld_h_b(); uint8_t ld_h_c(); uint8_t ld_h_d(); uint8_t ld_h_d8(); uint8_t ld_h_e(); uint8_t ld_h_h(); uint8_t ld_h_l();  uint8_t ld_hl_d16(); uint8_t ld_l_a(); uint8_t ld_l_abs_hl();  uint8_t ld_l_b(); uint8_t ld_l_c(); uint8_t ld_l_d(); uint8_t ld_l_e(); uint8_t ld_l_h(); uint8_t ld_l_l(); uint8_t ld_l_d8();  uint8_t ld_sp_d16(); uint8_t ldh_abs_a8_a(); uint8_t or_a(); uint8_t or_abs_hl();  uint8_t or_b();  uint8_t or_c(); uint8_t or_d();  uint8_t or_e(); uint8_t or_h(); uint8_t or_l(); uint8_t pop_bc(); uint8_t pop_de(); uint8_t pop_hl(); uint8_t prefix(); uint8_t push_bc(); uint8_t push_de();  uint8_t ret(); uint8_t ret_c(); uint8_t ret_nc(); uint8_t ret_nz(); uint8_t ret_z(); uint8_t reti();
    uint8_t rla();  uint8_t rlca(); uint8_t rlc_b(); uint8_t rra(); uint8_t rrca(); uint8_t rst_00h(); uint8_t rst_08h(); uint8_t rst_10h(); uint8_t rst_18h(); uint8_t sbc_a_a(); uint8_t sbc_a_abs_hl();  uint8_t sbc_a_b(); uint8_t sbc_a_c(); uint8_t sbc_a_d(); uint8_t sbc_a_d8(); uint8_t sbc_a_e(); uint8_t sbc_a_h(); uint8_t sbc_a_l();  uint8_t scf();
    uint8_t stop_d8(); uint8_t sub_a(); uint8_t sub_abs_hl(); uint8_t sub_b(); uint8_t sub_c(); uint8_t sub_d(); uint8_t sub_d8(); uint8_t sub_e(); uint8_t sub_h(); uint8_t sub_l();
