...
public:
    ...
    uint8_t ld_de_d16(); uint8_t ld_e_a();  uint8_t ld_e_abs_hl();  uint8_t ld_e_b(); uint8_t ld_e_c(); uint8_t ld_e_d(); uint8_t ld_e_d8(); uint8_t ld_e_e(); uint8_t ld_e_h(); uint8_t ld_e_l(); uint8_t ld_h_a(); uint8_t ld_h_abs_hl(); uint8_t ld_h_b(); uint8_t ld_h_c(); uint8_t ld_h_d(); uint8_t ld_h_d8(); uint8_t ld_h_e(); uint8_t ld_h_h(); uint8_t ld_h_l();  uint8_t ld_hl_d16(); uint8_t ld_l_a(); uint8_t ld_l_abs_hl();  uint8_t ld_l_b(); uint8_t ld_l_c(); uint8_t ld_l_d(); uint8_t ld_l_e(); uint8_t ld_l_h(); uint8_t ld_l_l(); uint8_t ld_l_d8();  uint8_t ld_sp_d16();  uint8_t rla();              uint8_t rlca();            uint8_t rra();             uint8_t rrca(); uint8_t sbc_a_abs_hl();  uint8_t sbc_a_b(); uint8_t sbc_a_c(); uint8_t sbc_a_d(); uint8_t sbc_a_e(); uint8_t sbc_a_h(); uint8_t sbc_a_l();  uint8_t scf();      uint8_t stop_d8(); uint8_t sub_a(); uint8_t sub_abs_hl(); uint8_t sub_b(); uint8_t sub_c(); uint8_t sub_d(); uint8_t sub_e(); uint8_t sub_h(); uint8_t sub_l();
