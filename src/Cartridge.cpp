#include "Cartridge.h"
/*
    Cons to the current Cartridge approach:
    - It is really annoying to add functionality since it is buried in so many different files.
    - It's also annoying having to have so many nested functions to do anything. I feel that
      having out parameters just to I can access ROM/RAM, within the "Cartridge" object (in
      the sense that the mapper/MBC is apart of the cartridge) is annoying. No real benefit to it either. 
    IDEA:
    - I think a better approach would be to have the mapper exposed completly at the cartridge level, rathan than being an
      object, make it a member of the cartridge class via a struct, where is soley contains data and no functions.
      - Then we create a single set of cpu r/w callbacks and use a switch to differentiate between the different
        MBC's.
*/
...
bool Cartridge::cpu_write(uint16_t addr, uint8_t data) {
    ...
    bool ram = false;
    if(p_mapper->cpu_map_write(addr, data, mapped_addr, ram)) 
    {
        if(ram)
        {
            cart_ram[mapped_addr] = data;
        }
        return true;
    }
    return false;
